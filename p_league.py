...
# ✅ FONCTION CORRIGÉE : Ajustement H2H intelligent
def ajuster_lambda_h2h(lambda_home, lambda_away, h2h_data):
    """
    Ajuste les lambda UNIQUEMENT si les H2H montrent une tendance significative
    N'applique plus systématiquement une réduction
    """
    if not h2h_data:
    ...
    avg_home_h2h = total_home / count
    avg_away_h2h = total_away / count

    # ✅ NOUVEAU : Ajustement intelligent selon l'écart
    ecart_home = avg_home_h2h - lambda_home
    ecart_away = avg_away_h2h - lambda_away
    
    # ✅ Ajustement proportionnel (plus l'écart est grand, plus on ajuste)
    # Mais limité à ±20% maximum
    poids_max = 0.20
    
    # Si H2H montre PLUS de buts que prévu → augmente
    # Si H2H montre MOINS de buts → réduit (mais doucement)
    if abs(ecart_home) > 0.3:  # Seuil de significativité
        poids_home = min(poids_max, abs(ecart_home) / lambda_home * 0.5)
        lambda_home += ecart_home * poids_home
    
    if abs(ecart_away) > 0.3:
        poids_away = min(poids_max, abs(ecart_away) / lambda_away * 0.5)
        lambda_away += ecart_away * poids_away

    print(f"🆚 Ajustement H2H: λ_home {lambda_home:.2f}, λ_away {lambda_away:.2f}")
    print(f"   Moyennes H2H: home={avg_home_h2h:.2f}, away={avg_away_h2h:.2f}")
    
    return lambda_home, lambda_away

# ✅ FONCTION CORRIGÉE : Simulation Monte-Carlo avec modèle Dixon-Coles
def simulation_match_montecarlo(stats_home, stats_away, h2h_data=None, n=20000):
    """
    Simulation Monte-Carlo corrigée avec modèle Dixon-Coles simplifié
    """
    print(f"🎲 Démarrage simulation Monte-Carlo avec {n} itérations...")
    
    # ✅ NOUVEAU MODÈLE : Force offensive × Faiblesse défensive adverse
    
    # 1️⃣ Calculer les forces/faiblesses relatives
    # Moyenne de la ligue (référence)
    ligue_avg_marqués = 1.5  # À ajuster selon la ligue
    ligue_avg_encaissés = 1.5
    
    # Force offensive (ratio par rapport à la moyenne)
    force_offensive_home = stats_home["moyenne_marques"] / ligue_avg_marqués
    force_offensive_away = stats_away["moyenne_marques"] / ligue_avg_marqués
    
    # Faiblesse défensive (ratio par rapport à la moyenne)
    faiblesse_def_home = stats_home["moyenne_encaisses"] / ligue_avg_encaissés
    faiblesse_def_away = stats_away["moyenne_encaisses"] / ligue_avg_encaissés
    
    # 2️⃣ Calcul des lambda selon le modèle Dixon-Coles simplifié
    lambda_home = ligue_avg_marqués * force_offensive_home * faiblesse_def_away
    lambda_away = ligue_avg_marqués * force_offensive_away * faiblesse_def_home
    
    print(f"🔢 λ modèle Dixon-Coles: Home={lambda_home:.2f}, Away={lambda_away:.2f}")
    
    # 3️⃣ Bonus domicile (historiquement ~30% de buts en plus)
    lambda_home *= 1.20
    lambda_away *= 0.90
    
    print(f"🏠 λ après bonus domicile: Home={lambda_home:.2f}, Away={lambda_away:.2f}")
    
    # 4️⃣ Ajustement par la forme récente
    forme_home = stats_home.get("total_points_6", 9) / 18
    forme_away = stats_away.get("total_points_6", 9) / 18
    
    lambda_home *= (0.9 + forme_home * 0.3)  # Entre 0.9 et 1.2
    lambda_away *= (0.9 + forme_away * 0.3)
    
    print(f"📊 λ après forme: Home={lambda_home:.2f}, Away={lambda_away:.2f}")
    
    # 5️⃣ Ajustement H2H (avec la nouvelle fonction corrigée)
    if h2h_data:
    ...
    return {
        "parametres_simulation": {
            ...
            "modele": "Dixon-Coles_simplifie_avec_bonus_domicile_et_forme"
        },
        "1x2": res_1x2,
...
def extract_confidence_percentage(analyse_ia):
    ...
    # ✅ NOUVEAUX PATTERNS pour le format avec saut de ligne et **CONFIANCE :**
    patterns_with_stars_newline = [
        r'\n\*\*CONFIANCE\s*:\*\*\s*(\d+)\s*%',
        r'\n\*\*Confiance\s*:\*\*\s*(\d+)\s*%',
        r'\n\*\*confiance\s*:\*\*\s*(\d+)\s*%',
        r'\n\*\*CONFIANCE\s*:\*\*\s*(\d+)\s*\%',
    ...
    # Patterns pour le format avec ** (ancien format)
    patterns_with_stars = [
        r'\*\*CONFIANCE\*\*\s*:\s*(\d+)\s*%',
        r'\*\*Confiance\*\*\s*:\s*(\d+)\s*%',
        r'\*\*confiance\*\*\s*:\s*(\d+)\s*%',
    ...
    # ✅ Essayer d'abord les nouveaux patterns avec saut de ligne
    for pattern in patterns_with_stars_newline:
        match = re.search(pattern, analyse_ia, re.IGNORECASE)
        if match:
            try:
                percentage = int(match.group(1))
                if 0 <= percentage <= 100:
                    print(f"📊 Pourcentage de confiance extrait (format **newline**) : {percentage}%")
                    return percentage
            except ValueError:
                continue
    
    # Essayer ensuite les patterns avec **
...
# ✅ NOUVELLES FONCTIONS D'EXTRACTION POUR LES AUTRES ÉLÉMENTS (SUPPORT DES TROIS FORMATS)
def extract_prediction_principale(analyse_ia):
    """
    Extrait la prédiction principale de l'analyse IA
    Support des trois formats y compris le nouveau avec saut de ligne
    """
    if not analyse_ia or isinstance(analyse_ia, str) and analyse_ia.startswith("❌"):
        return None
    
    # ✅ NOUVEAUX PATTERNS pour le format avec saut de ligne
    patterns_with_stars_newline = [
        r'\n\*\*PRÉDICTION PRINCIPALE\*\*\s*:\s*([^\n\r]+)',
        r'\n\*\*Prédiction principale\*\*\s*:\s*([^\n\r]+)',
        r'\n\*\*PREDICTION PRINCIPALE\*\*\s*:\s*([^\n\r]+)',
    ]
    
    # Patterns pour le format avec ** (ancien)
    patterns_with_stars = [
    ...
    # Patterns pour le format simple (ancien)
    patterns_simple = [
        r'PRÉDICTION PRINCIPALE\s*:\s*([^\n\r]+)',
        r'Prédiction principale\s*:\s*([^\n\r]+)',
        r'PREDICTION PRINCIPALE\s*:\s*([^\n\r]+)',
    ...
    for pattern in patterns_with_stars_newline:
        ...
        if match:
            ...
            print(f"🎯 Prédiction principale extraite (format **newline**) : {prediction}")
            return prediction
...
def extract_corners_prevu(analyse_ia):
    ...
    for pattern in patterns_with_stars_newline:
        ...
        if match:
            ...
            print(f"📐 Corners prévus extraits (format **newline**) : {corners}")
            return corners
...
def extract_tirs_cadres_prevu(analyse_ia):
    ...
    # ✅ NOUVEAUX PATTERNS pour le format avec saut de ligne
    patterns_with_stars_newline = [
        r'\n\*\*TIRS CADRÉS PRÉVUS\*\*\s*:\s*([^\n\r]+)',
        r'\n\*\*Tirs cadrés prévus\*\*\s*:\s*([^\n\r]+)',
        r'\n\*\*TIRS CADRES PREVUS\*\*\s*:\s*([^\n\r]+)',
    ]
    ...
    for pattern in patterns_with_stars_newline:
        ...
        if match:
            ...
            print(f"🎯 Tirs cadrés prévus extraits (format **newline**) : {tirs}")
            return tirs
...
def extract_scores_probables(analyse_ia):
    ...
    for pattern in patterns_with_stars_newline:
        ...
        if match:
            ...
            print(f"⚽ Scores probables extraits (format **newline**) : {scores}")
            return scores
...
def compare_teams_basic_stats(
    ...
    prediction_obj = {
        ...
        "type": "stats_brutes_avec_cotes_et_ia_avec_stats_detaillees_h2h_enrichi_tirs_confiance_scores_extraction_amelioree_v3_formats_dixon_coles_corriges",
    ...
    # 🎲 NOUVEAU : Calcul des probabilités statistiques Monte-Carlo CORRIGÉES (garde les données mais ne les inclut PAS dans le prompt)
    print(f"\n🎯 Calcul des probabilités statistiques Monte-Carlo CORRIGÉES...")
    ...
    print("\n🎯 PROBABILITÉS STATISTIQUES CORRIGÉES (Monte-Carlo + Dixon-Coles + bonus domicile + forme + H2H)")
    ...
    # ✅ NOUVELLES EXTRACTIONS AMÉLIORÉES AVEC SUPPORT DES TROIS FORMATS
    ...
    print("\n📚 Note : Statistiques brutes avec cotes + analyse IA DeepSeek avec retry + matchs complets avec stats détaillées + classement complet + H2H enrichi avec stats + confiance + scores + extraction améliorée des trois formats + PROBABILITÉS MONTE-CARLO CORRIGÉES INTÉGRÉES (non incluses dans le prompt IA) + modèle Dixon-Coles + ajustement H2H intelligent.")
...
def sauvegarder_stats_brutes_json(predictions_simples, date_str):
    ...
    data_complete = {
        "metadata": {
            ...
            "version_algorithme": "8.4 - STATISTIQUES BRUTES + FORMES 6/10 + POINTS CLASSEMENT + COTES + ANALYSE IA DEEPSEEK ENRICHIE + MATCHS COMPLETS AVEC STATS DÉTAILLÉES + CLASSEMENT COMPLET + H2H ENRICHI AVEC STATS + CONFIANCE EXTRAITE + SCORES + RETRY IA + EXTRACTION AMÉLIORÉE 3 FORMATS + PROBABILITÉS MONTE-CARLO CORRIGÉES + MODÈLE DIXON-COLES + AJUSTEMENT H2H INTELLIGENT",
            "total_predictions": total_predictions,
            "mode": "stats_brutes_avec_cotes_et_ia_complete_enrichie_retry_nouvelle_structure_avec_stats_detaillees_h2h_enrichi_confiance_extraite_scores_extraction_amelioree_3_formats_montecarlo_corriges_dixon_coles",
            "note": "Collecte des statistiques brutes complètes : moyennes, formes récentes (6 et 10 matchs), séries domicile/extérieur, classements avec points + cotes des bookmakers + analyse IA DeepSeek ENRICHIE avec matchs détaillés (nouvelle structure objet avec game_id, date, home_team, away_team, score, status, competition + STATS DÉTAILLÉES ESPN) + classement complet + confrontations directes H2H élargies AVEC STATS DÉTAILLÉES + pourcentage confiance EXTRAIT AUTOMATIQUEMENT + 2 scores probables + retry automatique IA + suppression 'match nul' + EXTRACTION AMÉLIORÉE support des 3 formats (**FORMAT**, FORMAT simple, **FORMAT :** avec saut de ligne) + PROBABILITÉS MONTE-CARLO CORRIGÉES (modèle Dixon-Coles + bonus domicile + forme + ajustement H2H intelligent)",
            ...
            "monte_carlo": {
                ...
                "iterations": 20000,
                "modele": "Dixon-Coles_simplifie_avec_bonus_domicile_et_forme",
                "calibrage": "force_offensive × faiblesse_defensive + bonus_domicile + forme_recente + ajustement_h2h_intelligent",
                "probabilites_calculees": ["1x2", "double_chance", "over_under", "btts", "resultat_total", "scores_probables"],
                "inclus_dans_prompt_ia": False,
                "corrections_v8_4": [
                    "🔧 Modèle Dixon-Coles : force_offensive × faiblesse_defensive",
                    "🏠 Bonus domicile : +20% home, -10% away",
                    "📊 Ajustement forme récente intégré",
                    "🆚 Ajustement H2H intelligent (non systématique)",
                    "❌ Suppression du bug de réduction systématique H2H"
                ]
            },
            "nouveautes_v8_4": [
                "🔧 CORRECTION MAJEURE : Modèle Monte-Carlo corrigé avec Dixon-Coles",
                "🎯 Force offensive × Faiblesse défensive adverse (vs ancien modèle incorrect)",
                "🏠 Bonus domicile calibré (+20% / -10%)",
                "📊 Intégration de la forme récente dans les lambda",
                "🆚 Ajustement H2H intelligent (seuil de significativité)",
                "❌ Suppression du bug de réduction systématique des lambda H2H",
                "✅ NOUVEAU : Support extraction format '\\n**CONFIANCE :** 68 %'",
                "✅ Support des 3 formats d'extraction IA complet",
                "📁 Nom de fichier simplifié maintenu",
                "✅ Maintien de toutes les fonctionnalités avancées v8.2-v8.3"
            ]
        },
        "statistiques_brutes_avec_ia_montecarlo_corriges": {
    ...
    print(f"📊 Total: {total_predictions} analyses complètes avec cotes + IA DeepSeek enrichie + retry + H2H enrichi avec stats + nouvelles fonctionnalités + extraction améliorée 3 formats + PROBABILITÉS MONTE-CARLO CORRIGÉES (Dixon-Coles + ajustement H2H intelligent)")
...
def git_commit_and_push(filepath):
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"📊 Statistiques brutes complètes du {datetime.now().strftime('%Y-%m-%d')} - Version 8.4 CORRIGÉE : STATS BRUTES + FORMES 6/10 + POINTS CLASSEMENT + COTES + ANALYSE IA DEEPSEEK ENRICHIE + MATCHS COMPLETS AVEC STATS DÉTAILLÉES ESPN + CLASSEMENT COMPLET + H2H ENRICHI AVEC STATS + CONFIANCE EXTRAITE + SCORES + RETRY IA + EXTRACTION AMÉLIORÉE 3 FORMATS + PROBABILITÉS MONTE-CARLO CORRIGÉES (MODÈLE DIXON-COLES + AJUSTEMENT H2H INTELLIGENT) + NOM FICHIER SIMPLIFIÉ"], check=True)
        subprocess.run(["git", "push"], check=True)
        print("✅ Statistiques brutes complètes avec cotes et analyse IA enrichie + stats détaillées ESPN + H2H enrichi + nouvelles fonctionnalités + extraction améliorée 3 formats + PROBABILITÉS MONTE-CARLO CORRIGÉES (Dixon-Coles + ajustement H2H intelligent) + nom fichier simplifié poussées avec succès sur GitHub.")
    except subprocess.CalledProcessError as e:
        print(f"❌ Erreur Git : {e}")

def main():
    print("📊 Bienvenue dans l'analyse v8.4 CORRIGÉE : STATISTIQUES BRUTES COMPLÈTES + ANALYSE IA DEEPSEEK ENRICHIE + RETRY + H2H ENRICHI AVEC STATS + CONFIANCE EXTRAITE + SCORES + EXTRACTION AMÉLIORÉE 3 FORMATS + PROBABILITÉS MONTE-CARLO CORRIGÉES (MODÈLE DIXON-COLES + AJUSTEMENT H2H INTELLIGENT) + NOM FICHIER SIMPLIFIÉ !")
    ...
    print("   🔧 - ✨ v8.2-v8.3 : EXTRACTION AMÉLIORÉE support des formats d'analyse IA")
    print("   ✅ - ✨ NOUVEAU v8.4 : Support extraction format '\\n**CONFIANCE :** 68 %'")
    print("   ✅ - ✨ NOUVEAU v8.4 : Support complet des 3 formats d'extraction")
    print("   🎲 - ✨ CORRIGÉ v8.4 : MODULE PROBABILITÉS MONTE-CARLO CORRIGÉ :")
    print("      🔧 Modèle Dixon-Coles : force_offensive × faiblesse_defensive")
    print("      🏠 Bonus domicile calibré (+20% home, -10% away)")
    print("      📊 Intégration forme récente dans les paramètres lambda")
    print("      🆚 Ajustement H2H intelligent avec seuil de significativité")
    print("      ❌ CORRECTION : Suppression du bug de réduction systématique H2H")
    print("      ▫️ 20 000 simulations par match maintenu")
    ...
    print("      ❌ Probabilités Monte-Carlo NON incluses dans le prompt IA (maintenu)")
    print("   ❌ IA ne prédit plus corners et tirs cadrés (champs gardés dans structure)")
    print("   📁 Nom de fichier simplifié prédiction-YYYY-MM-DD-analyse-ia.json (maintenu)")
    ...
    print("🔮 Prédictions basées sur l'analyse IA DeepSeek enrichie avec retry automatique + stats détaillées + H2H enrichi + nouvelles fonctionnalités + extraction améliorée 3 formats (SANS Monte-Carlo dans le prompt)")
    print("🔄 Mapping automatique des noms d'équipes conservé")
    print("🛑 Filtrage automatique des équipes avec forme nulle conservé")
    print("📊 Analyse pure et complète des statistiques brutes + IA enrichie + retry + H2H enrichi avec stats + extraction améliorée 3 formats + PROBABILITÉS MONTE-CARLO CORRIGÉES (hors prompt IA) des matchs du jour...\n")
    get_today_matches_filtered()
    print(f"\n📋 Résumé de la session:")
    print(f"   📊 {len(PREDICTIONS)} analyses complètes de statistiques brutes avec cotes et IA enrichie + stats détaillées ESPN + H2H enrichi + nouvelles fonctionnalités + extraction améliorée 3 formats + PROBABILITÉS MONTE-CARLO CORRIGÉES générées")
    ...
    print(f"   🔧 ✨ v8.2-v8.3 : Support robuste des formats d'analyse IA")
    print(f"   ✅ ✨ NOUVEAU v8.4 : Support extraction format '\\n**CONFIANCE :** 68 %'")
    print(f"   ✅ ✨ NOUVEAU v8.4 : Support complet des 3 formats d'extraction IA")
    print(f"   🎯 ✨ v8.2-v8.4 : Extraction automatique de prediction_principale, scores_probables")
    print(f"   🔧 ✨ CORRIGÉ v8.4 : PROBABILITÉS MONTE-CARLO avec modèle Dixon-Coles correct")
    print(f"   🏠 ✨ CORRIGÉ v8.4 : Bonus domicile calibré + forme récente intégrée")
    print(f"   🆚 ✨ CORRIGÉ v8.4 : Ajustement H2H intelligent (non systématique)")
    print(f"   ❌ ✨ CORRIGÉ v8.4 : Suppression du bug de réduction systématique H2H")
    print(f"   ❌ ✨ MAINTENU v8.4 : IA utilise uniquement stats + cotes + H2H (SANS probabilités Monte-Carlo)")
    print(f"   ❌ ✨ MAINTENU v8.4 : IA ne prédit plus corners et tirs cadrés (champs conservés dans structure)")
    if IGNORED_ZERO_FORM_TEAMS:
        print(f"   🚫 {len(set(IGNORED_ZERO_FORM_TEAMS))} équipes ignorées pour forme nulle")
    print("\n✨ Merci d'avoir utilisé le script v8.4 CORRIGÉ - Statistiques brutes complètes avec cotes et IA DeepSeek enrichie + retry + H2H enrichi avec stats + confiance extraite + scores + extraction améliorée 3 formats + PROBABILITÉS MONTE-CARLO CORRIGÉES (MODÈLE DIXON-COLES + AJUSTEMENT H2H INTELLIGENT) + NOM FICHIER SIMPLIFIÉ !")
